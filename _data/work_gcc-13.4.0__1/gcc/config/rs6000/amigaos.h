...
%{static|static-libgcc: %{!use-dynld: -lgcc -lgcc_eh} %{use-dynld: -lgcc} }%{!static:%{!static-libgcc:%{!shared:%{!shared-libgcc: %{!use-dynld: -lgcc -lgcc_eh} %{use-dynld: -lgcc}}%{shared-libgcc:-lgcc}}%{shared:%{shared-libgcc:-lgcc}%{!shared-libgcc:-lgcc}}}}"
  ...
  /* make amyclib the default */
  #undef MULTILIB_DEFAULTS
  #define MULTILIB_DEFAULTS             {"mcrt=amyclib"}
  #define CPP_AMIGA_DEFAULT_SPEC        "%{mcrt=default|!mcrt=*:%<mcrt=default -mcrt=amyclib} %(cpp_amyclib)"
  #define LINK_AMIGA_DEFAULT_SPEC       "%(link_amyclib)"
  #define STARTFILE_AMIGA_DEFAULT_SPEC  "%(startfile_amyclib)"
  #define ENDFILE_AMIGA_DEFAULT_SPEC    "%(endfile_amyclib)"
...
#define CPP_CLIB2_SPEC "" \
  "-idirafter %(base_sdk)clib2/include " \
  "-idirafter %(base_sdk)local/clib2/include"

#define LIB_SUBDIR_CLIB2_SPEC "" \
...
#define CPP_CLIB4_SPEC ""\
  "-idirafter %(base_sdk)clib4/include " \
  "-idirafter %(base_sdk)local/clib4/include"

#define LIB_SUBDIR_CLIB4_SPEC "" \
  "lib%(lib_subdir_type)"
...
#define STARTFILE_CLIB4_SPEC "" \
  "%{shared: %(base_sdk)clib4/%(lib_subdir_clib4)/shcrtbegin.o} " \
  "%{!shared: %(base_sdk)clib4/%(lib_subdir_clib4)/crtbegin.o} " \
  "%{!shared: %(base_sdk)clib4/%(lib_subdir_clib4)/crt0.o}"

#define ENDFILE_CLIB4_SPEC "" \
  "%{shared: %(base_sdk)clib4/%(lib_subdir_clib4)/shcrtend.o} " \
  "%{!shared: %(base_sdk)clib4/%(lib_subdir_clib4)/crtend.o}"

/* newlib */

#define CPP_NEWLIB_SPEC "" \
  "-idirafter %(base_sdk)newlib/include " \
  "-idirafter %(base_sdk)local/newlib/include"

#define LIB_SUBDIR_NEWLIB_SPEC "" \
  "lib%(lib_subdir_type)"
...
#define STARTFILE_NEWLIB_SPEC "" \
  "%{shared: %(base_sdk)newlib/%(lib_subdir_newlib)/shcrtbegin.o} " \
  "%{!shared: %(base_sdk)newlib/%(lib_subdir_newlib)/crtbegin.o}"

#define ENDFILE_NEWLIB_SPEC "" \
  "%{shared: %(base_sdk)newlib/%(lib_subdir_newlib)/shcrtend.o} " \
  "%{!shared: %(base_sdk)newlib/%(lib_subdir_newlib)/crtend.o}"
...
#define LINK_AMYCLIB_SPEC "" \
  ...
  "-L%(base_gcc)lib/gcc/ppc-amigaos/%(version)/amyclib/%(lib_subdir_amyclib) " \
...
// rwo #undef SUBTARGET_INIT_BUILTINS
// rwo #define SUBTARGET_INIT_BUILTINS amigaos_init_builtins ()
